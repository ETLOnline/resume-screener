export function extractTotalScoreFromMixedString(mixedString) {
  ...
  try {
    ...
    // Step 6: Access the 'Total Score' array
    if (dataObject && Array.isArray(dataObject['Total Score'])) {
      const totalScoreArray = dataObject['Total Score'];
      // Step 7: Find the last element containing the weighted average score
      const lastElement = totalScoreArray[totalScoreArray.length - 1];
      const match = lastElement.match(/Weighted Average Total Score: (\d+\.?\d*)/);
      if (match && match[1]) {
        return parseFloat(match[1]);
      } else {
        console.log("'Weighted Average Total Score' string not found in the last element of 'Total Score' array.");
        return null;
      }
    } else {
      console.log("'Total Score' key not found or is not an array in the extracted JSON data.");
      return null;
    }
